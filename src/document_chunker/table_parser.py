...
def _collect_table_region(lines: list[str], start_index: int) -> int:
    ...
    TableNormalizer.collect_region already uses (stop on blank/list_item)."""
    end_index = start_index
    while end_index < len(lines):
        stripped = lines[end_index].strip()
        if not stripped or match_list_item(stripped) is not None:
            break
        end_index += 1
    return end_index


def _rectangularize(rows: list[list[str]], num_columns: int) -> list[list[str]]:
    ...
    for row in rows:
        if len(row) < num_columns:
            ...
            row = list(row[: num_columns - 1]) + [" ".join(row[num_columns - 1 :])]
        else:
            row = list(row)
...
def parse_table_region(
    ...
    line)."""
    end_index = _collect_table_region(lines, start_index)
    ...
    block, consumed = TableNormalizer().build(classified, 0)
    if block is None or block.table is None:
        return None, start_index

    columns = block.table.header
    rows = _rectangularize(block.table.rows, len(columns))
